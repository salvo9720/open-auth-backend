...
WORKDIR /src
COPY ["open-auth-backend.csproj", "."]
RUN dotnet restore "./open-auth-backend.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./open-auth-backend.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./open-auth-backend.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "open-auth-backend.dll"]